...
function uninstallPlugin(url) {
    ...
    if (typeof found == "number") {
        if (confirm("Are you sure you would like to uninstall this plugin? Fusion OS will have to restart!")) {
...
pcodeInput.oninput = function () {
    if (pcodeInput.value.length == settings.lock.passcode.length) {
        if (pcodeInput.value == settings.lock.passcode) {
            ...
            sendNotification("Welcome to Fusion OS", "Fusion OS is currently running " + version)
        ...
        if (event.data[0] == "installPlugin") {
            ...
            if (app.permissions.includes("installPlugin")) {
                if (event.data[1] == "installPlugin") {
                    if (confirm("An app wants to install a plugin on Fusion OS. Plugins have FULL ACCESS to EVERYTHING on Fusion OS - only install plugins from this app if you truly trust it.\n\nWould you like to install the plugin at:\n" + event.data[2] + "\nThe app trying to install it is named " + app.name + ".")) {
...
console.log("%cThis is a browser feature %conly intended for developers. %cPasting code here could give bad people access to the entirety of Fusion OS, which may even include account credentials! Don't put anything here if you don't know what it does.", "font-family: sans-serif; font-size: 20px;", "color: red; font-family: sans-serif; font-size: 20px;", "font-family: sans-serif; font-size: 20px;");
